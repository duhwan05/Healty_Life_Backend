...
//UserEntity 객체의 정보로 사용자 인증 및 인가(권한부여)처리
@Getter
public class MyUserDetails implements UserDetails {
    ...
    public static UserDetails create(String userId, String userPw, Collection<? extends GrantedAuthority> authorities) {
        //UserEntity를 builder 사용해서 생성하고 MyUserDetails 객체 초기화
        //UserDetails : SpringSecurity의 인터페이스. 사용자 정보 제공 메소드 정의
        MyUserDetails myUserDetails = new MyUserDetails(UserEntity.builder()
                .userId(userId)
                .userPw(userPw)
                .build()); //User Entity 빌더 호출. UserEntity 객체 생성
